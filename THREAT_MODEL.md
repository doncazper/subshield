...
This threat model covers the SubShield browser client, stateless Cloudflare Worker, Reddit OAuth flow, and read-only Reddit API requests. It assumes Reddit has approved the OAuth client and the three production secrets are stored as encrypted Cloudflare secrets.
